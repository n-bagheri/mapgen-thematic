"""Alternate MapGen branch: aggregate Step 4, then simplify the groups.

Alt Step 5 changes only category identities in the untouched Step 4 raster.
Once those groups are approved, Alt Step 6 passes that aggregated raster
through the same area-generalization operations used by canonical Step 5.
Canonical artifacts are never used as the alternate branch's geography.
"""
...
from .alt_aggregate import (aggregation_fingerprint, effective_aggregation,
                            load_aggregation_review,
                            propose_complete_aggregation)
from .generalize import (LINE_JOIN_FAR_MM, LINE_JOIN_NEAR_MM,
                         SIMPLIFICATION_PRESETS, SIMPLIFY_MM, compute_scale,
                         drop_redundant_boundary_lines, generalize_area_raster,
                         line_length, merge_lines, preset_params, simplify_line)
...
ALT_STEP6_METHOD_VERSION = 4
ALT_STEP6_DEFAULT_PARAMS = {
    "method_version": ALT_STEP6_METHOD_VERSION,
...
def build_group_definitions(aggregation: dict, classes: list[dict]) -> list[dict]:
    ...
    for extra in aggregation.get("non_thematic_extra", []):
        add(extra["label"], [extra["index"]], False,
            "non-thematic class kept separate")
...
def materialize_alt_step5_output(out_dir: Path, aggregation: dict | None = None) -> dict:
    ...
    of geography until Alt Step 6 processes them.
    """
    aggregation_path = out_dir / "alt_aggregation.json"
    if aggregation is None:
        if not aggregation_path.exists():
            raise FileNotFoundError("run Alt Step 5 before materializing its output")
    ...
    if _source_digest(source, classes) != effective.get("source_digest"):
        raise RuntimeError("Step 4 changed after Alt Step 5; rerun Alt Step 5")
    ...
    imwrite(out_dir / "alt_group_map_source.png", (grouped + 1).astype(np.uint8))
    imwrite(out_dir / "alt_step5_aggregation_preview.png",
            _render_groups(grouped, definitions))
    (out_dir / "alt_groups.json").write_text(
    ...
    audit = {
        ...
        "output_raster": "alt_group_map_source.png",
        "output_groups": "alt_groups.json",
    ...
    (out_dir / "alt_step5_source_audit.json").write_text(
...
def run_alt_step5(image_path: Path, model: str | None = None,
    ...
    if not all(path.exists() for path in required):
        raise FileNotFoundError("run canonical Steps 1-4 before Alt Step 5")
    ...
    aggregation = {
        ...
        "water": ({"label": water[0]["label"],
                   "members": [int(cl["index"]) for cl in water]} if water else None),
        "groups": groups,
        "plain_thematic": [],
    ...
    # Alt Step 5 outputs. New multi-class proposals remain previews until the
    # user approves them in the review UI.
    if not review_required or (review and review.get("approved")):
        materialize_alt_step5_output(out_dir, aggregation)
    return {"out_dir": out_dir, "aggregation": aggregation, "audit": audit}


def load_alt_step6_params(out_dir: Path) -> dict:
    params = dict(ALT_STEP6_DEFAULT_PARAMS)
    path = out_dir / "alt_step6_params.json"
    if path.exists():
        saved = json.loads(path.read_text(encoding="utf-8"))
        if saved.get("method_version") == ALT_STEP6_METHOD_VERSION:
            params.update(saved)
    return params


def alt_step6_preset_params(level: int, base: dict | None = None) -> dict:
...
def alt_step6_preset_artifact_name(level: int, canonical_name: str) -> str:
    return f"alt_step6_preset_{int(level)}_{canonical_name.removeprefix('alt_')}"


def _cache_alt_step6_preset(out_dir: Path, level: int) -> None:
    for name in ALT_STEP6_PRESET_ARTIFACTS:
        shutil.copy2(out_dir / name, out_dir / alt_step6_preset_artifact_name(level, name))


def activate_alt_step6_preset(out_dir: Path, level: int) -> dict:
    level = max(1, min(5, int(level)))
    missing = [name for name in ALT_STEP6_PRESET_ARTIFACTS
               if not (out_dir / alt_step6_preset_artifact_name(level, name)).exists()]
    if missing:
        raise FileNotFoundError(f"Alt Step 6 preset {level} is not ready")
    for name in ALT_STEP6_PRESET_ARTIFACTS:
        shutil.copy2(out_dir / alt_step6_preset_artifact_name(level, name), out_dir / name)
    summary = json.loads((out_dir / "alt_step6_summary.json").read_text(encoding="utf-8"))
    (out_dir / "alt_step6_params.json").write_text(
...
def _generalize_lines(lines_in: list[dict], label_map: np.ndarray,
    ...
    """Run the canonical Step 5 line operations against the grouped raster."""
...
def _source_to_final_transition(source: np.ndarray, before_groups: np.ndarray,
    ...
    return {
        ...
        "source_raster": "label_map.png",
        "approved_unsimplified_group_raster": "alt_group_map_source.png",
        "simplified_group_raster": "alt_label_map_gen.png",
...
def run_alt_step6(image_path: Path, model: str | None = None,
    ...
    """Simplify the approved Alt Step 5 grouped raster for touch."""
    out_dir = runs_dir / image_path.stem
    aggregation_path = out_dir / "alt_aggregation.json"
    if not aggregation_path.exists():
    ...
    required = (out_dir / "alt_group_map_source.png", out_dir / "alt_groups.json",
                out_dir / "alt_step5_source_audit.json", out_dir / "lines.geojson")
    if not all(path.exists() for path in required):
        raise FileNotFoundError(
            "approve Alt Step 5 so its aggregated Step 4 raster exists before Alt Step 6")
    classes = json.loads((out_dir / "classes_final.json").read_text(
        encoding="utf-8"))["classes"]
    source = imread(out_dir / "label_map.png")[..., 0].astype(np.int16) - 1
    source_digest = _source_digest(source, classes)
    ...
    definitions = json.loads((out_dir / "alt_groups.json").read_text(
        encoding="utf-8"))["groups"]
    expected_definitions = build_group_definitions(aggregation, classes)
    if definitions != expected_definitions:
        raise RuntimeError("Alt Step 5 review changed; save it again before Alt Step 6")
    original = imread(out_dir / "alt_group_map_source.png")[..., 0].astype(np.int16) - 1
    expected_original, source_to_group = group_raster(source, definitions)
    if not np.array_equal(original, expected_original):
        raise RuntimeError("Alt Step 5 aggregated raster is stale; rerun Alt Step 5")

    requested = (load_alt_step6_params(out_dir) if params_override is None
    ...
    canonical_base = {}
    canonical_params_path = out_dir / "step5_params.json"
    ...
    params["method_version"] = ALT_STEP6_METHOD_VERSION
    params["input_raster"] = "alt_group_map_source.png"
    ...
    summary = {
        ...
        "source_step": 4,
        "input_raster": "alt_group_map_source.png",
    ...
    imwrite(out_dir / "alt_label_map_gen_preview.png", preview)
    (out_dir / "alt_groups.json").write_text(
    ...
    (out_dir / "alt_lines_gen.geojson").write_text(json.dumps({
        "type": "FeatureCollection", "coordinate_space": "map_area.png pixels, y down",
        "mm_per_px": round(mm_per_px, 4), "features": line_features,
    }), encoding="utf-8")
    (out_dir / "alt_step6_summary.json").write_text(
        json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    (out_dir / "alt_step6_transitions.json").write_text(
    ...
    imwrite(out_dir / "alt_step6_changes.png", changes)
    ...
    imwrite(out_dir / "alt_step6_debug.png", debug)
    return {"out_dir": out_dir, "summary": summary, "classes": group_classes,
            "polygons": len(regions), "transition": transition}


def run_alt_step6_presets(image_path: Path, model: str | None = None,
                          runs_dir: Path = Path("runs")) -> dict:
    out_dir = runs_dir / image_path.stem
    base = load_alt_step6_params(out_dir)
    params_path = out_dir / "alt_step6_params.json"
    ...
    for level in SIMPLIFICATION_PRESETS:
        run_alt_step6(image_path, model=model, runs_dir=runs_dir,
                      params_override=alt_step6_preset_params(level, base))
        _cache_alt_step6_preset(out_dir, level)
    summary = activate_alt_step6_preset(out_dir, selected)
    classes = json.loads((out_dir / "alt_classes_gen.json").read_text(
        encoding="utf-8"))["classes"]
    regions = json.loads((out_dir / "alt_regions_gen.geojson").read_text(
        encoding="utf-8"))["features"]
